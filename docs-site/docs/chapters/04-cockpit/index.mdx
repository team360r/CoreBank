...
class AppTheme {
  // CoreBank brand blue — our single seed colour
...
CoreBank will support both light and dark themes. The `ThemeMode` enum has three values: `light`, `dark`, and `system` (follows OS preference).
...
```dart title="lib/main.dart"
class CoreBankApp extends StatefulWidget {
  const CoreBankApp({super.key});

  @override
  State<CoreBankApp> createState() => _CoreBankAppState();
}

class _CoreBankAppState extends State<CoreBankApp> {
