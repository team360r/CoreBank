...
In Chapter 7 you will add a local SQLite database using Drift so FlightBank works offline. You will define type-safe tables, write reactive DAOs that stream changes to the UI, wire Drift into Riverpod, and handle schema migrations as the app evolves.
